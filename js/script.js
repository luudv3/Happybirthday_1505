    ...
    async function typeWriter(el, lines, speed = 55, lineDelay = 420, highlightLines = []){
      el.innerHTML = "";
      for(const line of lines){
        const span = document.createElement("span");
        const isHighlight = highlightLines.includes(line);
        if(isHighlight) span.className = el.id === "finalText" ? "final-highlight" : "highlight";
        el.appendChild(span);
    ...
    async function runIntro(){
      await typeWriter(
        ...
        ["Một ngày rất bình thường", "nhưng lại trở nên đặc biệt theo một cách riêng"],
        64,
        550
      );
      await sleep(850);
    ...
    function burstGiftPhotos(){
      const imgs = ["img/hue10.jpg","img/hue11.jpg","img/hue12.jpg","img/hue4.jpg","img/hue14.jpg","img/hue6.jpg"];
      imgs.forEach((src, i) => {
        const card = document.createElement("div");
        card.className = "gift-mini-photo";
        card.style.setProperty("--x", `${(Math.random() - .5) * 260}px`);
        card.style.setProperty("--y", `${-70 - Math.random() * 180}px`);
        card.style.setProperty("--r", `${(Math.random() - .5) * 70}deg`);
    ...
    const memories = [
      { img:"img/hue10.jpg", text:"Một khoảnh khắc rất bình thường, nhưng lại đáng nhớ" },
      { img:"img/hue11.jpg", text:"Có những điều chỉ cần nhìn thôi cũng thấy nhẹ lòng" },
      { img:"img/hue12.jpg", text:"Cậu của ngày hôm nay vẫn rất xinh đẹp" },
      { img:"img/hue4.jpg", text:"Mỗi tấm ảnh là một chút dịu dàng được giữ lại" },
      { img:"img/hue14.jpg", text:"Có những nụ cười làm ngày bình thường trở nên khác hơn" },
      { img:"img/hue6.jpg", text:"Mong cậu luôn được yêu thương theo cách thật nhẹ nhàng" }
    ];
    ...
    function startAfterMemory(){
     ...
     const photoList = [
      "img/hue.jpg","img/hue1.jpg","img/hue2.jpg","img/hue3.jpg","img/hue4.jpg","img/hue5.jpg","img/hue6.jpg","img/hue7.jpg",
      "img/hue8.jpg","img/hue9.jpg","img/hue10.jpg","img/hue11.jpg","img/hue12.jpg","img/hue13.jpg","img/hue14.jpg","img/hue15.jpg","img/hue16.jpg","img/hue17.jpg","img/hue18.jpg","img/hue19.jpg"
    ];
    let photoIndex = 0;
    ...
    async function startMainStory(){
      ...
      await typeWriter(
        ...
        46,
        420,
        ["ở đâu đó vẫn có người", "luôn mong cậu bình yên mỗi ngày"]
      );
      await sleep(650);
    ...
    noBtn.addEventListener("pointerdown", e => { e.preventDefault(); moveNoBtn(); });
    ...
    function pastelFirework(x = Math.random() * innerWidth, y = Math.random() * innerHeight * .55 + 40){
    ...
    function launchPhotoSky(count = 18){
      for(let i = 0; i < count; i++){
        setTimeout(() => {
          ...
          card.style.setProperty("--rot", `${(Math.random() - .5) * 18}deg`);
          card.style.setProperty("--spin", `${(Math.random() - .5) * 42}deg`);
          card.style.setProperty("--drift-x", `${(Math.random() - .5) * 110}px`);
  ...
  document.querySelectorAll(".falling, .wish-star, .sky-memory-photo, .pastel-firework, .gift-mini-photo")
